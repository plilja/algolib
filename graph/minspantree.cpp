...
inline pair<int, int> smallest_first(pair<int, int> &p)
{
    return {std::min(p.first, p.second), std::max(p.first, p.second)};
}

template <typename T>
priority_queue<T, vector<T>, std::greater<T>> reverse_priority_queue()
{
...
SpanningTree prim(vector<pair<pair<int, int>, double>> &edges, int nr_of_nodes)
{
    ...
    auto pq = reverse_priority_queue<pair<double, pair<int, int>>>();
    ...
    while (!pq.empty()) {
        ...
        if (p.second.first != -1)
            tree_edges.push_back(smallest_first(p.second));
