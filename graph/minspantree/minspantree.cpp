...
bool prim(std::vector<std::pair<int, int> > &ans, double &ansCost, int nrOfNodes, std::vector<std::pair<std::pair<int, int>, double> > &edges)
{
	...
	for(std::vector<std::pair<std::pair<int, int>, double> >::iterator it=edges.begin(); it!=edges.end(); it++)
	{
		u=it->first.first;
		v=it->first.second;
		weight=it->second;
	...
	double *bestCost=new double[nrOfNodes];
	...
	pq.insert(std::make_pair(0.0, 0)); //begin with node 0 
	bestCost[0]=0;
	int nodeCounter=0;
	...
	while(pq.size()!=0)
	{
		...
		for(std::list<std::pair<int, double> >::iterator it=nodes[p.second].edges.begin(); it!=nodes[p.second].edges.end(); it++)
		{
