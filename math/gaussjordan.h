...
template <typename T>
int findPivot(uint column, uint next_row, const vector<vector<T>* > &A)
{
    uint dim=A.size();
...
template <typename T>
int gaussjordan(vector<vector<T> > A, vector<T> y, vector<T> &x)
{
    Code c=isZeroSystem(A, y);
    if(c==INCONSISTENT)
        return INCONSISTENT;
    else if(c==MULTIPLE)
        return MULTIPLE;
...
template <typename T>
int gaussjordanplus(vector<vector<T> > A, vector<T> y, vector<std::pair<bool,T> > &x)
{
	uint dim=A.size();
    Code c=isZeroSystem(A, y);
    if(c==INCONSISTENT)
    ...
    {
        x = vector<std::pair<bool, T> >(dim, std::pair<bool, T>(false, T()));
        return MULTIPLE;
    }
