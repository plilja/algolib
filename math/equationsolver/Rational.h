...
class Rational
{
	...
	public:
		...
		//friend std::iostream& operator << (std::iostream &os, const Rational &rhs); 
...
template <typename T>
Rational<T>::Rational(T value) : n(value), d(1)
{
}
...
Rational<T> Rational<T>::operator + (const Rational<T> &rhs) const
{
    ...
    //	T newDen = lcm(d, rhs.den());
	//T newNum = n * (newDen / d) + rhs.num() * (newDen / rhs.den());
...
/*std::iostream& operator << (std::iostream &os, const Rational<int> &rhs)
{
	os<<rhs.num()<<"/"<<rhs.den();
	return os;
} */
